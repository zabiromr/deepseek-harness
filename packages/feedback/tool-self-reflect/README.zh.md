...
证据还必须指向工作本身。无论发生什么，会话都会累积事件——它被配置、一个轮次开启、一个步骤启动——而请求这项工作的那条消息同样不是工作。因此至少要有一个被引用事件是 `assistant/message`、`tool/call` 或 `tool/result`：模型说了什么、调用了什么，以及调用返回了什么。这个集合刻意保持小而封闭；插件自有的事件不可单独作为证据，因为工具无从得知某个贡献类型的含义，因此依赖它的教训应当与产生它的那次调用一并引用。

schema 直接指明这些序号从何而来：`session_event_search`，并以 `event_types` 过滤出引用所需的调用与结果。若不指明，模型就会转而去读会话日志文件——那是一个拼接的 Zstandard 容器，并将分块运行打包进单行存储，既耗费大量步骤，天真读取时还会解析出错误的序号。
