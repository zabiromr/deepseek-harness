...
若某次捕获的正文在同一作用域中已经存在，该调用会被拒绝，并指明应当去 confirm 的那条教训。记录成本低廉且自身没有身份，因此模型重述已知内容时只会新增一份副本，而不是抬升原有教训的地位，而每份副本都要为同一句话付出摘要预算。比较是精确匹配：改写过的教训属于不同的主张，而判断其差异程度并非一项检查该做的裁量。

`confirm` 与 `contradict` 还要求本会话确实被展示过该教训。二者都在主张一个源自实际使用该教训的结果——它再次被证实，或它产生了误导——而从未触达过该教训的会话并没有这样的结果可报告。教训以两种方式抵达会话，且两者都算数：摘要将它渲染进系统提示词，召回工具将它返回进转录。若没有这项检查，一次误发的重述就会以「可解析且指向工作、却与该教训毫无关系」的证据，让一条可靠的教训退役。

schema 不仅指明这些序号从何而来，还给出完整的调用方式，而非只给工具名。`session_event_search` 要求 `query` 非空，因此仅被告知「用 session_event_search」的模型会组装出非法调用，转而退回去读会话日志文件——那是一个拼接的 Zstandard 容器，并将分块运行打包进单行存储：既耗费大量步骤，天真读取时会解析出错误的序号，还曾因派生 shell 反射加载解压类型而触发行为式杀毒启发式告警。因此 schema 明确给出要用的 query（取自工作本身：用过的工具名、读过的路径）、`event_types` 过滤条件，并说明这已经足够。
