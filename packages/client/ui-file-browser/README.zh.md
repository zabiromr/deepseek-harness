...
当部署的用户面向本地工作区工作、并希望在对话记录旁就地编辑文件时选择它。它要求部署挂载文件系统 provider：缺少它时会话 Remote 会以 `unsupported` 拒绝 `file.list`、`file.read` 与 `file.write`，浏览器随即显示列举失败。仅供自动化的界面不应加入它，因为那些界面根本不挂载浏览器半边。
...
- **保存是无条件的。** 编辑器通过 `file.write` 写入整个缓冲区，不做冲突检测，因此打开之后在磁盘上被改动的文件会被直接覆盖且无警告。
- **没有文件大小或二进制防护。** 浮层会把 `file.read` 返回的任何内容按文本渲染，因此超大或二进制文件会被整体载入浏览器。
