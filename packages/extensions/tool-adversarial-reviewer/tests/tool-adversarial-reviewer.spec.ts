...
const TOOL_NAME = 'tool-adversarial-reviewer'
 ...
 */
function call(ctx: Context, args: unknown = { action: 'review' }) {
...
describe(`${TOOL_NAME} execution`, () => {
  it('reports success without consulting its action argument', async () => {
    ...
    const reviewed = await call(ctx, { action: 'review' })
  ...
  it('renders one fixed line naming what the tool is for', async () => {
    const { ctx } = await setup()
    expect(text(await call(ctx))).toBe('Self-critique via subagent review.: done')
  })
})
