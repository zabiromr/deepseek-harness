...
A capture whose body is already stored in the same scope is refused, naming the lesson to confirm instead. Recording is cheap and has no identity of its own, so a model restating what it already knows adds a copy rather than raising the original’s standing, and each copy costs digest budget for the same sentence. The comparison is exact: a reworded lesson is a different claim, and deciding how different is not a check’s judgement to make.

`confirm` and `contradict` additionally require that this session was shown the lesson. Both claim an outcome from having acted on it — it proved right again, or it misled — and a session the lesson never reached has no such outcome to report. A lesson reaches a session two ways and both count: the digest renders it into the system prompt, and the recall tool returns it into the transcript. Without the check, a stray restatement retires a sound lesson on evidence that resolves and names work while saying nothing about that lesson.

The schema names where those sequence numbers come from, and gives the whole call rather than the tool's name alone. `session_event_search` requires a non-empty `query`, so a model told only to "use session_event_search" composes an invalid call and falls back to the session log files — a concatenated Zstandard container packing chunk runs into single rows, which costs many steps, resolves to the wrong numbers when read naively, and has tripped behavioural antivirus heuristics by spawning a shell to load a decompression type reflectively. The schema therefore states the query to use (something from the work itself: a tool name, a path read), the `event_types` filter, and that this is sufficient.
